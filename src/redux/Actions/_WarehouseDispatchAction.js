...
export const fetchWarehouseDispatchAllList = createAsyncThunk(
  ...
  async (payload) => {
    ...
    // Make the Axios GET request with the headers
    const response = await axios.get(`${base_url}/warehouse-dispatch/`, {
      headers,
    });
...
//
//
export const fetchWarehouseDispatch = createAsyncThunk(
...
export const sortByAZWarehouseDispatch = createAsyncThunk(
  ...
  async ({ sortOrder, page }, { getState }) => {
    ...
    try {
      ...
      const apiUrl = `${base_url}/warehouse-dispatch/?order=${sortOrder}&limit=${perPage}&offset=${
        (page - 1) * perPage
      }`;
